    ...
    async def stream_research(self, query: str):
        try:
            ...
            if not config_status["valid"]:
                ...
                self.wfile.write(f"data: {json.dumps({'error': error_msg, 'type': 'error'})}\n\n".encode())
                return
            ...
            while not research_task.done():
                ...
                if current_event_count > last_event_count:
                    for event in progress_event.events[last_event_count:]:
                        self.wfile.write(f"data: {json.dumps({'type': 'progress', 'data': event})}\n\n".encode())
                        self.wfile.flush()
            ...
            self.wfile.write(f"data: {json.dumps({'type': 'complete', 'data': final_data})}\n\n".encode())
            self.wfile.flush()
            
        except Exception as e:
            logger.error(f"Streaming research error: {e}")
            self.wfile.write(f"data: {json.dumps({'error': str(e), 'type': 'error'})}\n\n".encode())